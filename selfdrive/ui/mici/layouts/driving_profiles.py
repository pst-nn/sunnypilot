...
PROFILE_DETAILS = {
  DrivingProfile.STOCK_DASHCAM: "stock control / comma records",
  DrivingProfile.SUNNY_TACC: "sunny steer / tesla speed",
  DrivingProfile.SUNNY_LONG_EXPERIMENTAL: "WARNING: tesla AEB unavailable",
}

PROFILE_CONFIRM_TITLES = {
  DrivingProfile.STOCK_DASHCAM: "slide for\nstock / dashcam",
  DrivingProfile.SUNNY_TACC: "slide for\nsunny + tesla tacc",
  DrivingProfile.SUNNY_LONG_EXPERIMENTAL: "slide for sunny long\nAEB unavailable",
}
...
class DrivingProfileHomeButton(Widget):
  ...
  def __init__(self):
    ...
    # MiciHomeLayout owns the large home-screen touch target.
  ...
  def _render(self, _):
    ...
    kicker_rect = rl.Rectangle(self._rect.x + 18, self._rect.y + 2, self._rect.width - 56, 18)
    profile_rect = rl.Rectangle(self._rect.x + 18, self._rect.y + 18, self._rect.width - 60, 36)
    arrow_rect = rl.Rectangle(self._rect.x + self._rect.width - 42, self._rect.y, 30, self._rect.height)

    gui_label(kicker_rect, "DRIVING PROFILE", font_size=16, color=rl.Color(180, 180, 184, 255), font_weight=FontWeight.MEDIUM)
    gui_label(profile_rect, label, font_size=27, font_weight=FontWeight.BOLD)
    gui_label(arrow_rect, ">", font_size=32, color=rl.Color(205, 205, 208, 255), alignment=rl.GuiTextAlignment.TEXT_ALIGN_CENTER)
...
class DrivingProfilesLayout(NavScroller):
  ...
  def _refresh_cards(self):
    ...
    for profile, button in self._buttons.items():
      active_prefix = "ACTIVE / " if profile == current else ""
      button.set_value(active_prefix + PROFILE_DETAILS[profile])
